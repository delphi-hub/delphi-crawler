...
import de.upb.cs.swt.delphi.crawler.discovery.maven.LegacyMavenCrawlActor.{DiscoverMetadata, ProcessMetadata, ProcessNew, StartDiscover}
...
class LegacyMavenCrawlActor(baseUri: Uri, elasticActor: ActorRef) extends Actor with ActorLogging {
...
object LegacyMavenCrawlActor {
  def props(baseURL: Uri, elasticActor: ActorRef): Props = Props(new LegacyMavenCrawlActor(baseURL, elasticActor))
