...
class PomFileReadActor(configuration: Configuration) extends Actor with ActorLogging{
  ...
  private def getParentPomModel(implicit pomContent: Model): Option[Model] = {
    ...
    if (parentDef != null && parentDef.getGroupId != null && parentDef.getArtifactId != null && parentDef.getVersion != null){
      val parentIdentifier = MavenIdentifier(Some(configuration.mavenRepoBase.toString), parentDef.getGroupId,
        parentDef.getArtifactId, Some(parentDef.getVersion))
  ...
  private def buildParentIdentifier(implicit pomContent:Model): MavenIdentifier = {
    MavenIdentifier(Some(configuration.mavenRepoBase.toString), pomContent.getParent.getGroupId,
      pomContent.getParent.getArtifactId, Some(pomContent.getParent.getVersion))
  }
  ...
  : Try[ArtifactDependency] = {
    ...
    Try {
      ...
      ArtifactDependency(MavenIdentifier(Some(configuration.mavenRepoBase.toString), groupId, artifactId, Some(version)), scope)
    }
  }
  ...
  : Option[String] = {
    ...
    if (variableParts(0).equals("project") || variableParts(0).equals("pom")) {
      result = variableParts(1) match {
        ...
        case "version" => identifier.version
